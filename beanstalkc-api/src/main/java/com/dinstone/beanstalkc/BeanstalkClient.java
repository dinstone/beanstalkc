...
/**
 * {@link BeanstalkClient} is a full amount interface, mainly used for management.
 * 
 * @author guojf
 * 
...
public interface BeanstalkClient extends JobConsumer, JobProducer {
    ...
    /**
     * The peek commands let the client inspect a job in the system, return the delayed job with the shortest delay
     * left.
     * 
     * @return
     */
    ...
    /**
     * The stats-job command gives statistical information about the specified job if it exists.<br>
     * The stats-job data is a YAML file representing a single dictionary of strings to scalars. It contains these
     * keys:<br>
     * - "id" is the job id<br>
     ...
     * - "time-left" is the number of seconds left until the server puts this job into the ready queue. This number is
     * only meaningful if the job is reserved or delayed. If the job is reserved and this amount of time elapses before
     * its state changes, it is considered to have timed out.<br>
     * - "timeouts" is the number of times this job has timed out during a reservation.<br>
     * - "releases" is the number of times a client has released this job from a reservation.<br>
     * - "buries" is the number of times this job has been buried.<br>
     * - "kicks" is the number of times this job has been kicked.<br>
     * 
     * @param jobId
     * 
     * @return a sequence of bytes. It is a YAML file with statistical information represented a dictionary.
     */
    public Map<String, String> statsJob(long jobId);

    /**
     * The stats-tube command gives statistical information about the specified tube if it exists.<br>
     * The stats-tube data is a YAML file representing a single dictionary of strings to scalars. It contains these
     * keys:<br>
     * - "name" is the tube's name.<br>
     * - "current-jobs-urgent" is the number of ready jobs with priority < 1024 in this tube.<br>
     * - "current-jobs-ready" is the number of jobs in the ready queue in this tube.<br>
     * - "current-jobs-reserved" is the number of jobs reserved by all clients in this tube.<br>
     ...
     * - "current-waiting" is the number of open connections that have issued a reserve command while watching this tube
     * but not yet received a response.<br>
     * 
     * @param tubeName
     * 
     * @return
     ...
     * The stats data for the system is a YAML file representing a single dictionary of strings to scalars. It contains
     * these keys:<br>
     ...
     * - "cmd-list-tubes-watched" is the cumulative number of list-tubes-watched commands. <br>
     ...
     * - "current-producers" is the number of open connections that have each issued at least one put command.<br>
     * - "current-workers" is the number of open connections that have each issued at least one reserve command.<br>
     * - "current-waiting" is the number of open connections that have issued a reserve command but not yet received a
     * response.<br>
     ...
     * - "rusage-utime" is the accumulated user CPU time of this process in seconds and microseconds.<br>
     * - "rusage-stime" is the accumulated system CPU time of this process in seconds and microseconds.<br>
     * - "uptime" is the number of seconds since this server started running.<br>
     * - "binlog-oldest-index" is the index of the oldest binlog file needed to store the current jobs<br>
     * - "binlog-current-index" is the index of the current binlog file being written to. If binlog is not active this
     * value will be 0<br>
     * - "binlog-max-size" is the maximum size in bytes a binlog file is allowed to get before a new binlog file is
     * opened<br>
     * 
     * @return
     */
